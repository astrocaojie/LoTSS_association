...
PARENT_SUPPORT_THRESHOLD_TOLERANCE = float(
    PARENT_LINK_DEFAULTS["support_thresholds"]["threshold_match_tolerance"]
)

# Retrieval bounds only; final pair acceptance uses the configured gap limit.
PARENT_SEARCH_GAP_FLOOR_BEAM = 6.0
PARENT_SEARCH_GAP_CAP_BEAM = 18.0
...
# Parent fields retain geometry, host evidence, conflict resolution, and review status.
...
PARENT_DIAGNOSTIC_COLUMNS = [
    ...
    "n_final_candidates",
    "n_accepted_before_quality_filter",
...
def parent_link_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    defaults = copy.deepcopy(PARENT_LINK_DEFAULTS)
    out = dict(defaults)
    out.update(raw)
    for nested in (
        "endpoint_thresholds",
    ...
    # Feature switches are copied into the resolved Stage-2 view so helper
    # functions called directly (outside ``run_parent_links``) observe the
    # same validated switches as the production pipeline.
    out["use_stage2_relative_scale_constraints"] = feature_enabled(config, "use_stage2_relative_scale_constraints")
    out["use_stage2_endpoint_filtering"] = feature_enabled(config, "use_stage2_endpoint_filtering")
    out["use_artifact_penalties_layer2"] = feature_enabled(config, "use_artifact_penalties_layer2")
    out["use_midpoint_host_support"] = feature_enabled(config, "use_midpoint_host_support")
    out["use_lobe_peak_host_contradiction"] = feature_enabled(config, "use_lobe_peak_host_contradiction")
    out["use_multithreshold_contour"] = feature_enabled(config, "use_multithreshold_contour")
    return out
...
def _endpoint_thresholds(cfg: dict[str, Any] | None) -> dict[str, Any]:
    """Return the complete endpoint-threshold view for helper callers."""

    return (cfg or PARENT_LINK_DEFAULTS).get("endpoint_thresholds", PARENT_LINK_DEFAULTS["endpoint_thresholds"])
...
def _point_like(
    ...
    if not np.all(np.isfinite(values)):
        return False
    thresholds = _endpoint_thresholds(cfg)
    return bool(
        int(values[0]) == 1
        and values[1] < float(thresholds["compact_las_max_beam"])
        and values[2] < float(thresholds["compact_major_max_beam"])
        and values[3] < float(thresholds["compact_area_max_beam"])
        and values[4] < float(thresholds["compact_axis_ratio_max"])
    )
...
def _hard_point_source_veto(
    ...
    beam_like = bool(
        ...
        and int(n_gauss) == 1
        and (
            np.isfinite(major_axis)
            and major_axis < float(thresholds["point_major_max_beam"])
            or np.isfinite(las)
            and las < float(thresholds["point_las_max_beam"])
        )
        and area3 < float(thresholds["point_area_max_beam"])
        and axis_ratio < float(thresholds["point_axis_ratio_max"])
        and not connected3
        and not connected25
        and not bridge
    )
...
def _hard_compact_veto(
    ...
    if (
        ...
        and n_gauss <= 2
        and las < float(thresholds["compact_las_max_beam"])
        and area3 < float(thresholds["compact_area_max_beam"])
        and major_axis < float(thresholds["compact_major_max_beam"])
        and axis_ratio < float(thresholds["compact_axis_ratio_max"])
    ):
        reasons.append("beam_like_compact_source")
    if (
        np.all(np.isfinite([n_gauss, las, area3, axis_ratio]))
        and int(n_gauss) == 1
        and las < float(thresholds["singleton_las_max_beam"])
        and area3 < float(thresholds["singleton_area_max_beam"])
        and axis_ratio < float(thresholds["singleton_axis_ratio_max"])
    ):
        reasons.append("beam_like_compact_source")
    if (
        np.all(np.isfinite([major_axis, minor_axis, area3]))
        and major_axis < float(thresholds["observed_major_max_beam"])
        and minor_axis < float(thresholds["observed_minor_max_beam"])
        and area3 < float(thresholds["compact_area_max_beam"])
    ):
        reasons.append("beam_like_compact_source")
...
def _noise_artifact_veto(
    ...
    thresholds = _endpoint_thresholds(cfg)
    reasons: list[str] = []
    peak_threshold = float(thresholds["noise_peak_snr_min"])
    area_threshold = float(thresholds["noise_area_min_beam"])
    if not np.isfinite(peak_snr):
        reasons.append("peak_snr_missing")
    elif peak_snr < peak_threshold:
        reasons.append("peak_snr_below_threshold")
    if not np.isfinite(area3):
        reasons.append("area_3sigma_beam_missing")
    elif area3 < area_threshold:
        reasons.append("area_3sigma_beam_below_threshold")
...
def _extended_evidence_count(
    ...
    thresholds = (cfg or {}).get("endpoint_thresholds", {}) or {}
    signals = [
        safe_float(row.get("LAS_beam"), 0.0) >= float(thresholds["extended_las_min_beam"]),
        np.isfinite(area3) and area3 >= float(thresholds["extended_area_min_beam"]),
        np.isfinite(major_axis) and major_axis >= float(thresholds["extended_major_min_beam"]),
        axis_ratio >= float(thresholds["extended_axis_ratio_min"]),
        safe_float(row.get("n_gaussians"), 1.0) >= float(thresholds["extended_n_gaussians_min"]),
        atype in allowed_types,
    ]
    return int(sum(bool(value) for value in signals))


def _near_extended_lobe_candidate(
    row: pd.Series,
    area3: float,
    axis_ratio: float,
    major_axis: float,
    ...
    las = safe_float(row.get("LAS_beam"), float("nan"))
    thresholds = (cfg or {}).get("endpoint_thresholds", {}) or {}
    evidence = _extended_evidence_count(row, area3, axis_ratio, major_axis, cfg)
    strong_multi = bool(
        np.all(np.isfinite([n_gauss, area3, las]))
        and n_gauss >= float(thresholds["strong_multi_min_gaussians"])
        and area3 >= float(thresholds["strong_multi_min_area_beam"])
        and las
        >= float(thresholds["extended_las_min_beam"])
        - float(thresholds["strong_multi_las_margin_beam"])
    )
...
def _artifact_environment(row: pd.Series, groups: pd.DataFrame, cfg: dict[str, Any], config: dict[str, Any]) -> dict[str, Any]:
    ...
    pixel_scale = resolve_pixel_scale_arcsec(row.get("pixel_scale_arcsec"), config, context="parent-link artifact pixel scale")
    ...
    artifact_cfg = cfg["artifact_environment_thresholds"]
    bright = work.sort_values("_flux_for_artifact", ascending=False).head(int(artifact_cfg["bright_reference_count"]))
    best_dist = float("inf")
    best_radial = 0.0
    best_ratio = 0.0
    ...
    radius_beam = float(cfg["fragment_density_radius_beam"])
    ...
    density = float(max(local_count, 0) / (np.pi * radius_beam**2))
    score = 0.0
    reasons: list[str] = []
    if (
        best_dist <= float(cfg["bright_source_near_beam"])
        and best_ratio >= float(cfg["bright_source_flux_ratio_min"])
        and best_radial >= float(cfg["bright_source_radial_min"])
    ):
        score += float(artifact_cfg["radial_base_score"]) + float(artifact_cfg["radial_alignment_bonus"]) * best_radial
        reasons.append("near_radial_to_bright_source")
    if (
        best_dist <= float(artifact_cfg["very_close_distance_beam"])
        and best_ratio >= float(artifact_cfg["very_bright_flux_ratio"])
    ):
        score += float(artifact_cfg["very_close_score"])
        reasons.append("very_close_to_bright_source")
    if local_count >= int(cfg["fragment_count_artifact"]):
        score += float(artifact_cfg["crowded_fragment_score"])
        reasons.append("crowded_fragment_region")
    if _artifact_flags(row):
        score += float(artifact_cfg["artifact_flag_score"])
...
def _source_morph(
    ...
    # Classify local groups before admitting them as double-lobe endpoints.
    artifact = safe_float(artifact_env.get("artifact_environment_score"), 0.0) >= float(cfg["artifact_veto_score"])
    peak_snr = safe_float(row.get("peak_snr"), float("nan"))
    area_for_veto = area3
    hard_point, point_reason, beam_like_single = _hard_point_source_veto(row, area_for_veto, axis_ratio, major_axis, cfg)
    hard_compact, compact_reason = _hard_compact_veto(row, area_for_veto, axis_ratio, major_axis, minor_axis, cfg)
    noise_veto, noise_reason = _noise_artifact_veto(row, area_for_veto, mask_area, peak_snr, artifact, cfg)
    point = _point_like(row, area_for_veto, axis_ratio, major_axis, cfg) or hard_point or hard_compact
    atype = _association_type(row)
    quality = _association_quality(row)
    thresholds = cfg["endpoint_thresholds"]
    extension_signals = [
        safe_float(row.get("LAS_beam"), 0.0) >= float(thresholds["extended_las_min_beam"]),
        safe_float(row.get("n_gaussians"), 1.0) >= float(thresholds["extended_n_gaussians_min"]),
        np.isfinite(area3) and area3 >= float(thresholds["extended_area_min_beam"]),
    ...
    hard_gates_pass = bool(
        ...
        and peak_snr >= float(thresholds["noise_peak_snr_min"])
        and np.isfinite(area3)
        and area3 >= float(thresholds["noise_area_min_beam"])
    )
    ...
    singleton_extended = bool(
        ...
        and sum(bool(value) for value in extension_signals) >= 2
        and hard_gates_pass
    )
    reject: list[str] = []
    ...
    if artifact:
        ...
        lobe_reject.append("noise_artifact_veto")
    if not allowed_type and not singleton_extended:
        reject.append(f"bad_type:{atype}")
        lobe_reject.append("not_extended_enough_for_lobe")
    if not allowed_quality and not singleton_extended:
        reject.append(f"bad_quality:{quality}")
        lobe_reject.append("not_extended_enough_for_lobe")
    if sum(bool(value) for value in extension_signals) < 2:
        reject.append("insufficient_lobe_signals")
        lobe_reject.append("not_extended_enough_for_lobe")
    n_gauss = safe_float(row.get("n_gaussians"), float("nan"))
    enough_self_extended = bool(
        safe_float(row.get("LAS_beam"), 0.0) >= float(thresholds["singleton_las_max_beam"])
        or (
            np.isfinite(area3)
            and area3
            >= float(thresholds["extended_area_min_beam"])
            + float(thresholds["self_extended_area_margin_beam"])
        )
        or (
            np.isfinite(major_axis)
            and major_axis >= float(thresholds["self_extended_major_min_beam"])
        )
        or axis_ratio
        >= float(thresholds["point_axis_ratio_max"])
        + float(thresholds["self_extended_axis_ratio_margin"])
        or (
            np.isfinite(n_gauss)
            and n_gauss >= float(thresholds["strong_multi_min_gaussians"])
        )
    ...
    near_candidate, near_reason = _near_extended_lobe_candidate(row, area3, axis_ratio, major_axis, blocked, cfg)
    is_lobe = bool(
        not blocked
        and (singleton_extended or (allowed_type and allowed_quality))
        and sum(bool(value) for value in extension_signals) >= 2
        and hard_gates_pass
    )
...
def _threshold_index(
    segmentation: Any,
    threshold: float,
    tolerance: float = PARENT_SUPPORT_THRESHOLD_TOLERANCE,
) -> int | None:
    ...
    if abs(float(thresholds[idx]) - float(threshold)) <= float(tolerance):
        return idx
    return None
...
def _label_at_group(
    ...
    idx = _threshold_index(segmentation, threshold, tolerance)
    if idx is None or not hasattr(segmentation, "labels_by_threshold"):
        return 0
    labels = segmentation.labels_by_threshold[idx]
    height, width = labels.shape
    centroid_x = safe_float(group.get("centroid_x"), float("nan"))
    centroid_y = safe_float(group.get("centroid_y"), float("nan"))
    if not np.isfinite(centroid_x) or not np.isfinite(centroid_y):
        return 0
    x = int(round(np.clip(centroid_x, 0, width - 1)))
    y = int(round(np.clip(centroid_y, 0, height - 1)))
...
def _neighbor_support_features(
    segmentation: Any,
    group_a: pd.Series,
    group_b: pd.Series,
    config: dict[str, Any] | None = None,
) -> dict[str, Any]:
    resolved_cfg = parent_link_config(config or {})
    support_cfg = resolved_cfg["support_thresholds"]
    use_contour = bool(resolved_cfg["use_multithreshold_contour"])
    sigma3 = float(support_cfg["sigma_3"])
    sigma25 = float(support_cfg["sigma_2p5"])
    threshold_tolerance = float(support_cfg["threshold_match_tolerance"])
    bridge_low_snr = float(support_cfg["bridge_low_snr"])
    bridge_high_snr = float(support_cfg["bridge_high_snr"])
    label3_a = _label_at_group(segmentation, group_a, sigma3, threshold_tolerance)
    label3_b = _label_at_group(segmentation, group_b, sigma3, threshold_tolerance)
    label25_a = _label_at_group(segmentation, group_a, sigma25, threshold_tolerance)
    label25_b = _label_at_group(segmentation, group_b, sigma25, threshold_tolerance)
    same3 = bool(use_contour and label3_a > 0 and label3_a == label3_b)
    same25 = bool(use_contour and label25_a > 0 and label25_a == label25_b)
    values = _line_snr_values(segmentation, group_a, group_b)
    if values.size:
        low_fraction = float(np.nanmean(values >= bridge_low_snr))
        high_fraction = float(np.nanmean(values >= bridge_high_snr))
        bridge_support = bool(
            low_fraction >= float(support_cfg["bridge_low_fraction"])
            or high_fraction >= float(support_cfg["bridge_high_fraction"])
        )
        ridge = float(
            np.clip(
                float(support_cfg["ridge_low_weight"]) * low_fraction
                + float(support_cfg["ridge_high_weight"]) * high_fraction,
                0.0,
                1.0,
            )
        )
...
def _box_size_beam(seed: pd.Series, group: pd.Series, config: dict[str, Any]) -> float:
    ...
    if box is None:
        return float("nan")
    pixel_scale = resolve_pixel_scale_arcsec(group.get("pixel_scale_arcsec"), config, context="parent-link box pixel scale")
    beam_arcsec = compute_beam_size_arcsec(config)
...
def _pair_boundary_features(record: dict[str, Any], seed_a: pd.Series, seed_b: pd.Series, group_a: pd.Series, group_b: pd.Series, segmentation: Any, config: dict[str, Any]) -> dict[str, Any]:
    ...
    rescue_cfg = parent_link_config(config)["rescue_thresholds"]
    near_boundary = bool(
        np.isfinite(gap)
        and gap >= 0
        and (
            gap <= float(rescue_cfg["near_gap_beam"])
            or (np.isfinite(gap_mean) and gap_mean <= float(rescue_cfg["gap_to_mean_box_max"]))
            or (np.isfinite(gap_min) and gap_min <= float(rescue_cfg["gap_to_min_box_max"]))
        )
    )
    support = _neighbor_support_features(segmentation, group_a, group_b, config)
    support.update(
        {
...
def _precompute_neighbor_support(frame: pd.DataFrame, groups: pd.DataFrame, segmentation: Any, config: dict[str, Any]) -> pd.DataFrame:
    ...
    local_cfg = parent_link_config(config)
    pixel_scale = resolve_pixel_scale_arcsec(
        groups.get("pixel_scale_arcsec", pd.Series([None])).iloc[0], config, context="parent-link neighbor pixel scale"
    )
    support_cfg = local_cfg["support_thresholds"]
    max_gap_pix = float(support_cfg["neighbor_search_radius_beam"]) * compute_beam_size_arcsec(config) / max(pixel_scale, 1e-6)
    ...
    for idx_i, idx_j in _candidate_search_pairs(
        work,
        max_gap_pix,
        padding_factor=float(local_cfg["candidate_search_padding_factor"]),
    ):
        ...
        support = _neighbor_support_features(segmentation, group_i, group_j, config)
        connected = bool(
            support["same_3sigma_region_as_neighbor"]
            or support["same_2p5sigma_region_as_neighbor"]
            or support["bridge_snr_support"]
            or safe_float(support["ridge_continuity_score_pair"], 0.0)
            >= float(support_cfg["neighbor_ridge_min"])
        )
        if not connected:
            continue
        extended_i = _extended_evidence_count(seed_i, safe_float(seed_i.get("area_3sigma_beam"), float("nan")), safe_float(seed_i.get("axis_ratio"), 1.0), safe_float(seed_i.get("major_axis_beam"), float("nan")), local_cfg) >= 2
        extended_j = _extended_evidence_count(seed_j, safe_float(seed_j.get("area_3sigma_beam"), float("nan")), safe_float(seed_j.get("axis_ratio"), 1.0), safe_float(seed_j.get("major_axis_beam"), float("nan")), local_cfg) >= 2
    ...
    for _, row in groups.iterrows():
        ...
        if "pixel_scale_arcsec" not in work:
            work["pixel_scale_arcsec"] = resolve_pixel_scale_arcsec(
                ...
                config,
                context="parent-link source pixel scale",
            )
        ...
        records.append(
            {
                ...
                "peak_snr": peak_snr,
                "peak_snr_threshold": float(cfg["endpoint_thresholds"]["noise_peak_snr_min"]),
                "area_3sigma_beam_threshold": float(cfg["endpoint_thresholds"]["noise_area_min_beam"]),
                "association_quality": _association_quality(work),
                "association_type": _association_type(work),
                "is_compact_singleton": bool(parent.get("is_compact_singleton", False)),
...
def _symmetry_scores(record: dict[str, Any], cfg: dict[str, Any]) -> tuple[float, float, float, float]:
    flux_score = _balance_score(safe_float(record.get("flux_ratio"), float("inf")), float(cfg["max_flux_ratio"]))
    size_score = _balance_score(safe_float(record.get("size_ratio"), float("inf")), float(cfg["max_size_ratio"]))
    weights = cfg["symmetry_weights"]
    midpoint_symmetry = float(np.clip(safe_float(record.get("midpoint_symmetry_score"), 0.5), 0.0, 1.0))
    symmetry = float(
        float(weights["axis_alignment"]) * safe_float(record.get("axis_alignment_score"), 0.0)
        + float(weights["facing"]) * safe_float(record.get("facing_score"), 0.0)
        + float(weights["midpoint_symmetry"]) * midpoint_symmetry
...
def _parent_union(row: dict[str, Any], seed_a: pd.Series, seed_b: pd.Series, group_a: pd.Series, group_b: pd.Series, config: dict[str, Any]) -> dict[str, Any]:
    ...
    pixel_scale = resolve_pixel_scale_arcsec(group_a.get("pixel_scale_arcsec"), config, context="parent-link union pixel scale")
    beam_arcsec = compute_beam_size_arcsec(config)
    las_arcsec = float(np.hypot(x1 - x0, y1 - y0) * pixel_scale)
    area_beam = float(max((x1 - x0 + 1) * (y1 - y0 + 1), 0.0) / max(_shared_beam_area_pix(pixel_scale, beam_arcsec), 1e-6))
    return {
        "parent_bbox_xmin": x0,
        "parent_bbox_xmax": x1,
        "parent_bbox_ymin": y0,
...
def _classify_pair(record: dict[str, Any], seed_a: pd.Series, seed_b: pd.Series, cfg: dict[str, Any]) -> tuple[bool, str]:
    ...
    use_endpoint_filtering = bool(cfg["use_stage2_endpoint_filtering"])
    use_relative_scale = bool(cfg["use_stage2_relative_scale_constraints"])
    use_artifact_layer2 = bool(cfg["use_artifact_penalties_layer2"])
    ...
    if safe_float(record.get("box_gap_beam_robust"), float("inf")) > float(cfg["max_box_gap_beam"]):
        return False, "box_gap_too_large"
    if safe_float(record.get("center_distance_beam"), float("inf")) > float(cfg["max_center_distance_beam"]):
        return False, "center_distance_too_large"
    if safe_float(record.get("axis_alignment_score"), 0.0) < float(cfg["min_axis_alignment"]):
        return False, "geometry_axis_alignment_too_low"
    if safe_float(record.get("facing_score"), 0.0) < float(cfg["min_facing_score"]):
        return False, "geometry_facing_too_low"
    if use_relative_scale:
        if safe_float(record.get("flux_ratio"), float("inf")) > float(cfg["max_flux_ratio"]):
            return False, "asymmetric_or_point_lobe_pair"
        if safe_float(record.get("size_ratio"), float("inf")) > float(cfg["max_size_ratio"]):
            return False, "asymmetric_or_point_lobe_pair"
        if safe_float(record.get("symmetry_score"), 0.0) < float(cfg["min_symmetry_score"]):
...
def _near_boundary_strong_support(
    record: dict[str, Any],
    seed_a: pd.Series,
    seed_b: pd.Series,
    cfg: dict[str, Any] | None = None,
    ...
    cfg = cfg or {}
    rescue_cfg = cfg["rescue_thresholds"]
    ridge_min = float(rescue_cfg["support_ridge_min"])
    return bool(
        bool(record.get("same_3sigma_region_as_neighbor", False))
        or bool(record.get("same_2p5sigma_region_as_neighbor", False))
        or safe_float(record.get("ridge_continuity_score_pair"), 0.0) >= ridge_min
        or bool(record.get("bridge_snr_support", False))
        or (
            n1 >= float(rescue_cfg["min_gaussians_each"])
            and n2 >= float(rescue_cfg["min_gaussians_each"])
        )
        or (n1 >= float(rescue_cfg["extended_gaussians_min"]) and ext2)
        or (n2 >= float(rescue_cfg["extended_gaussians_min"]) and ext1)
        or (
            area1 >= float(rescue_cfg["min_area_each_beam"])
            and area2 >= float(rescue_cfg["min_area_each_beam"])
        )
    )
...
def _classify_near_boundary_rescue(record: dict[str, Any], seed_a: pd.Series, seed_b: pd.Series, cfg: dict[str, Any]) -> tuple[bool, str]:
    use_endpoint_filtering = bool(cfg["use_stage2_endpoint_filtering"])
    use_relative_scale = bool(cfg["use_stage2_relative_scale_constraints"])
    if use_endpoint_filtering and not _rescue_endpoint_allowed(record, seed_a, seed_b, bool(cfg["use_artifact_penalties_layer2"])):
        return False, "endpoint_vetoed_or_not_extended_for_near_boundary_rescue"
    rescue_cfg = cfg["rescue_thresholds"]
    if safe_float(record.get("axis_alignment_score"), 0.0) < float(rescue_cfg["axis_alignment_min"]):
        return False, "near_boundary_axis_alignment_too_low"
    if safe_float(record.get("facing_score"), 0.0) < float(rescue_cfg["facing_min"]):
        return False, "near_boundary_facing_too_low"
    if use_relative_scale:
        if safe_float(record.get("flux_ratio"), float("inf")) > float(rescue_cfg["max_flux_ratio"]):
            return False, "near_boundary_flux_ratio_too_high"
        if safe_float(record.get("size_ratio"), float("inf")) > float(rescue_cfg["max_size_ratio"]):
            return False, "near_boundary_size_ratio_too_high"
    if not _near_boundary_strong_support(record, seed_a, seed_b, cfg):
...
def _make_pair_edges(cutout_id: str, groups: pd.DataFrame, morph: pd.DataFrame, segmentation: Any, config: dict[str, Any]) -> pd.DataFrame:
    # Stage 2 searches for extended endpoint pairs with compatible geometry.
    cfg = parent_link_config(config)
    cfg["use_stage2_relative_scale_constraints"] = feature_enabled(config, "use_stage2_relative_scale_constraints")
    cfg["use_stage2_endpoint_filtering"] = feature_enabled(config, "use_stage2_endpoint_filtering")
    cfg["use_artifact_penalties_layer2"] = feature_enabled(config, "use_artifact_penalties_layer2")
    cfg["use_multithreshold_contour"] = feature_enabled(config, "use_multithreshold_contour")
    ...
    if "pixel_scale_arcsec" not in work:
        work["pixel_scale_arcsec"] = resolve_pixel_scale_arcsec(
            None, config, context="parent-link pair pixel scale"
        )
    work = work.merge(morph[["association_group_id", "robust_bbox", "robust_bbox_source", "is_parent_seed", "is_compact_singleton"]], on="association_group_id", how="left")
    beam_arcsec = compute_beam_size_arcsec(config)
    pixel_scale = resolve_pixel_scale_arcsec(work["pixel_scale_arcsec"].iloc[0], config, context="parent-link pair pixel scale")
    ...
    candidate_search_gap_beam = max(
        float(cfg["max_box_gap_beam"]),
        min(max_box_size_beam, PARENT_SEARCH_GAP_CAP_BEAM),
        PARENT_SEARCH_GAP_FLOOR_BEAM,
    )
    max_gap_pix = candidate_search_gap_beam * beam_arcsec / max(pixel_scale, 1e-6)
    records: list[dict[str, Any]] = []
    for pair_index, (idx_i, idx_j) in enumerate(
        ...
        artifact_pair = max(safe_float(seed_i.get("artifact_environment_score"), 0.0), safe_float(seed_j.get("artifact_environment_score"), 0.0)) if bool(cfg["use_artifact_penalties_layer2"]) else 0.0
        score_weights = cfg["score_weights"]
        lobe_score = float(
            float(score_weights["symmetry"]) * symmetry
            + float(score_weights["gap"]) * np.clip(1.0 - safe_float(record.get("box_gap_beam_robust"), 999.0) / max(float(cfg["max_box_gap_beam"]), 1e-6), 0.0, 1.0)
            + float(score_weights["midpoint_core"]) * (1.0 if bool(record.get("core_candidate_near_midpoint", False)) else 0.0)
        )
...
def _query_position_hosts(
    ...
    any_failed = False
    status = "not_queried"
    for catalogue in host_cfg["catalog_priority"]:
...
def _score_lobe_peak_hosts(
    ...
    scoring = cfg["lobe_peak_host_scoring"]
    snr_scale = max(float(scoring["snr_scale"]), 1e-6)
    records: list[dict[str, Any]] = []
    for _, host in raw_hosts.iterrows():
        ...
        if np.isfinite(w1snr):
            det += float(scoring["w1_detection_weight"]) * min(w1snr / snr_scale, 1.0)
        if np.isfinite(w2snr):
            det += float(scoring["w2_detection_weight"]) * min(w2snr / snr_scale, 1.0)
        if det == 0.0 and np.isfinite(safe_float(host.get("W1"), float("nan"))):
            det = float(scoring["catalogued_w1_fallback"])
        score = float(
            float(scoring["closeness_weight"]) * closeness
            + float(scoring["detection_weight"]) * det
        )
        quality = (
            "high"
            if score >= float(scoring["high_score_min"])
            and sep <= float(scoring["high_radius_fraction"]) * radius_arcsec
            else "medium"
            if score >= float(scoring["medium_score_min"])
            else "low"
        )
        records.append(
            {
...
def _apply_limits(candidates: pd.DataFrame, cfg: dict[str, Any]) -> pd.DataFrame:
    ...
    max_group = int(cfg["max_parent_candidates_per_group"])
    max_cutout = int(cfg["max_parent_candidates_per_cutout"])
    kept: list[int] = []
    counts: dict[str, int] = {}
...
def _parent_radio_evidence(row: pd.Series | dict[str, Any], cfg: dict[str, Any]) -> list[str]:
    evidence: list[str] = []
    if bool(cfg.get("use_multithreshold_contour", True)) and (_truthy(row.get("same_3sigma_region_as_neighbor")) or _truthy(row.get("same_2p5sigma_region_as_neighbor"))):
...
def classify_parent_acceptance(row: pd.Series | dict[str, Any], config: dict[str, Any]) -> dict[str, Any]:
    ...
    # Host evidence cannot replace the independent radio-evidence gate.
    cfg = parent_link_config(config)
    cfg["use_midpoint_host_support"] = feature_enabled(config, "use_midpoint_host_support")
    cfg["use_lobe_peak_host_contradiction"] = feature_enabled(config, "use_lobe_peak_host_contradiction")
    cfg["use_artifact_penalties_layer2"] = feature_enabled(config, "use_artifact_penalties_layer2")
    cfg["use_stage2_endpoint_filtering"] = feature_enabled(config, "use_stage2_endpoint_filtering")
    cfg["use_multithreshold_contour"] = feature_enabled(config, "use_multithreshold_contour")
    required_host_rank = min_host_quality_rank(cfg["host_support"])
    ...
    off_axis = safe_float(row.get("best_host_perp_offset_beam"), 0.0) > float(
        cfg["host_support"]["geometry"]["medium_max_perp_offset_beam"]
    )
    far_midpoint = safe_float(row.get("best_host_sep_midpoint_arcsec"), 0.0) > safe_float(row.get("host_search_radius_arcsec"), np.inf)
    radio_evidence = _parent_radio_evidence(row, cfg)
    geometry_ok = _truthy(row.get("double_lobe_geometry_pass"))
    endpoints_ok = True
    if bool(cfg["use_stage2_endpoint_filtering"]):
        endpoints_ok = bool(
    ...
    artifact = bool(cfg["use_artifact_penalties_layer2"]) and safe_float(row.get("artifact_environment_score_pair"), 0.0) >= float(cfg["artifact_veto_score"])

    host_support = bool(
        bool(cfg["use_midpoint_host_support"])
        and host_evidence == "supports_double_lobe"
        and _quality_rank(str(row.get("host_quality", "none"))) >= required_host_rank
    )
    evidence = [*radio_evidence, *(["midpoint_host"] if host_support else [])]
    host_required = bool(cfg["host_support"].get("require_host_for_parent_link", True))
    host_at_lobe_peak = bool(cfg["use_lobe_peak_host_contradiction"] and lobe_peak and host_evidence != "supports_double_lobe")
    no_host = host_evidence in {"needs_host_check", "no_midpoint_host", "not_checked"} or str(row.get("host_status", "")) in {
        "disabled",
        "no_plausible_host",
        "not_queried",
    }
    ...
    elif not radio_evidence:
        acceptance = "geometry_only_visual_candidate"
        reason = "geometry_only_no_independent_parent_evidence"
    elif host_required and not host_support:
        reason = reason or "host_required_for_parent_link"
    else:
        acceptance = "accepted_high_confidence_parent"
        reason = "+".join(evidence)

    final_quality = str(row.get("parent_candidate_quality", ""))
    if acceptance == "accepted_high_confidence_parent" and not host_required and no_host:
        quality_cfg = cfg["candidate_quality_thresholds"]
        final_quality = (
            "high"
            if safe_float(row.get("symmetry_score"), 0.0) >= float(quality_cfg["high_symmetry_min"])
            else "medium"
        )
    ...
    return {
        ...
        "parent_candidate_quality": final_quality,
...
def resolve_parent_conflicts(edges: pd.DataFrame, config: dict[str, Any]) -> pd.DataFrame:
    ...
    # Resolve competing parents so each local group belongs to at most one.
    ...
    def canonical_key(row: pd.Series) -> str:
        candidate_id = str(row.get("parent_candidate_id", "")).strip()
        if candidate_id and candidate_id.lower() != "nan":
            return candidate_id
        pair = sorted(
            (
                str(row.get("local_group_id_1", "")),
                str(row.get("local_group_id_2", "")),
            )
        )
        return "|".join(pair)

    accepted_rows["_canonical_candidate_key"] = accepted_rows.apply(canonical_key, axis=1)
    accepted_rows = accepted_rows.sort_values(
        ["_rank_tuple", "_canonical_candidate_key"],
...
def run_parent_links(
    ...
    # Endpoint filtering precedes geometry, host scoring, and conflict resolution.
    cfg = parent_link_config(config)
    cfg["use_midpoint_host_support"] = feature_enabled(config, "use_midpoint_host_support")
    cfg["use_lobe_peak_host_contradiction"] = feature_enabled(config, "use_lobe_peak_host_contradiction")
    cfg["use_artifact_penalties_layer2"] = feature_enabled(config, "use_artifact_penalties_layer2")
    host_cfg = cfg["host_support"]
    required_host_rank = min_host_quality_rank(host_cfg)
    if not bool(cfg["parent_seed"].get("enabled", True)):
        empty_edges = pd.DataFrame(columns=PARENT_EDGE_DEBUG_COLUMNS)
        empty_morph = pd.DataFrame(columns=SOURCE_MORPH_TABLE_COLUMNS)
        empty_log = pd.DataFrame(columns=HOST_QUERY_LOG_COLUMNS)
        return ParentLinkResult(
            candidates=pd.DataFrame(columns=PARENT_CANDIDATE_COLUMNS),
            edges_debug=empty_edges,
            host_candidates=pd.DataFrame(columns=PARENT_HOST_CANDIDATE_COLUMNS),
            host_query_log=empty_log,
            diagnostics=_diagnostics(cutout_id, empty_morph, empty_edges, empty_log),
            needs_visual_check=pd.DataFrame(columns=["cutout_id", "record_type", "object_id", "reason", "priority", "details"]),
            source_morph_table=empty_morph,
        )
    ...
    if "pixel_scale_arcsec" not in groups:
        if groups.empty:
            ...
            groups["pixel_scale_arcsec"] = resolve_pixel_scale_arcsec(
                local_components["pixel_scale_arcsec"].iloc[0], config, context="parent-link pixel scale"
            )
        else:
            groups["pixel_scale_arcsec"] = resolve_pixel_scale_arcsec(None, config, context="parent-link pixel scale")
    ...
    host_enabled = bool(host_cfg.get("enabled", True))
    if not edges.empty:
        pass_edges = edges[edges["double_lobe_geometry_pass"].astype(bool)].copy()
        for idx, edge in pass_edges.iterrows():
            parent = edge.copy()
            if not host_enabled:
                edges.loc[idx, "host_status"] = "disabled"
                edges.loc[idx, "host_evidence"] = "host_support_disabled"
                edges.loc[idx, "host_quality"] = "none"
                edges.loc[idx, "query_status"] = "disabled"
                continue
            ...
            peak_radius = min(max(float(cfg["lobe_peak_host_radius_arcsec_min"]), beam_arcsec), float(cfg["lobe_peak_host_radius_arcsec_max"]))
            ...
            for role, group in [("lobe1_peak", g1), ("lobe2_peak", g2)]:
                ...
                scored_peak = _score_lobe_peak_hosts(
                    raw_peak,
                    pra,
                    pdec,
                    peak_radius,
                    cutout_id,
                    str(parent.get("parent_candidate_id")),
                    role,
                    cfg,
                )
            ...
            midpoint_support = bool(cfg["use_midpoint_host_support"]) and _quality_rank(host_quality) >= required_host_rank
            lobe_peak_contradiction_enabled = bool(cfg["use_lobe_peak_host_contradiction"])
            rejection = ""
            if lobe_peak_contradiction_enabled and lobe1_found and lobe2_found:
                ...
                if artifact_pair >= float(cfg["artifact_suspicious_score"]):
                    quality = "suspicious"
                else:
                    quality_cfg = cfg["candidate_quality_thresholds"]
                    quality = (
                        "high"
                        if host_quality == "high"
                        and safe_float(parent.get("symmetry_score"), 0.0)
                        >= float(quality_cfg["high_symmetry_min"])
                        else "medium"
                    )
                ...
                quality = (
                    "needs_host_check"
                    if safe_float(parent.get("symmetry_score"), 0.0)
                    >= float(quality_cfg["needs_host_check_symmetry_min"])
                    else "suspicious"
                )
            ...
            final_score = safe_float(parent.get("lobe_pair_score"), 0.0) + float(host_cfg["host_score_weight"]) * host_score
            updates = {
                ...
                "needs_visual_check": bool(quality != "rejected"),
    ...
    accepted_mask = edges["parent_acceptance_class"].astype(str).eq("accepted_high_confidence_parent") if not edges.empty else pd.Series(dtype=bool)
    candidate_mask = accepted_mask & edges["parent_candidate_quality"].astype(str).eq("high") if not edges.empty else pd.Series(dtype=bool)
            ...
            [
                {
                    ...
                    "n_final_candidates": int(((acceptance == "accepted_high_confidence_parent") & (quality == "high")).sum()),
                    "n_accepted_before_quality_filter": int((acceptance == "accepted_high_confidence_parent").sum()),
