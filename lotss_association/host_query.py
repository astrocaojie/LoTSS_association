...
class HostQueryClient:
    ...
    def _load_cache(self) -> pd.DataFrame:
        if self.cache_path.exists():
            try:
                ...
                get_logger().warning("Could not read host parquet cache %s: %s", self.cache_path, exc)
            ...
            with self.jsonl_path.open("r", encoding="utf-8") as handle:
                for line_number, line in enumerate(handle, start=1):
                    ...
                    try:
                        ...
                        get_logger().warning(
                            "Ignoring malformed host cache row %s:%d: %s", self.jsonl_path, line_number, exc
                        )
            if rows:
                return self._repair_cache(pd.DataFrame(rows))
            get_logger().warning("Host JSONL cache %s contained no valid rows", self.jsonl_path)
        ...
        for _, row in frame.iterrows():
            ...
            if needs_repair and isinstance(raw_text, str) and raw_text:
                try:
                    ...
                    get_logger().warning(
                        "Could not repair host cache row %s (query_id=%s): %s",
                        self.cache_path,
                        record.get("query_id", ""),
                        exc,
                    )
    ...
    def save_cache(self) -> None:
        ...
        try:
            ...
            parquet_error = exc
            get_logger().warning("Could not write host parquet cache %s: %s", self.cache_path, exc)
        ...
        try:
            ...
            jsonl_error = exc
            get_logger().error("Could not write host JSONL cache %s: %s", self.jsonl_path, exc)
        if parquet_error is not None and jsonl_error is not None:
            raise RuntimeError(
                f"Unable to persist host query cache to {self.cache_path} or {self.jsonl_path}"
            ) from jsonl_error
    ...
    def query_catalogue(self, ra: float, dec: float, radius_arcsec: float, catalogue: str) -> HostQueryResult:
        ...
        try:
            ...
            get_logger().warning(
                "Host catalogue query failed (catalogue=%s, query_id=%s): %s", catalogue, query_id, exc
            )
