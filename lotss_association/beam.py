...
# Beam-aware distances use the configured restoring beam as their scale.
...
def beam_covariance_from_config(config: dict[str, Any] | None = None) -> np.ndarray:
    ...
    pa = safe_float(beam.get("pa_deg"), 0.0)
...
def elliptical_beam_distance(delta_xy_arcsec: np.ndarray | tuple[float, float], beam_cov: np.ndarray) -> float:
    ...
    # Project the elliptical beam along the separation vector.
...
def beam_area_arcsec2(
    ...
    if config is not None:
        beam = (config or {}).get("beam", {}) or {}
        raw_major = safe_float(beam.get("major_arcsec"), float("nan"))
        raw_minor = safe_float(beam.get("minor_arcsec"), float("nan"))
        if not np.isfinite(raw_major) or raw_major <= 0 or not np.isfinite(raw_minor) or raw_minor <= 0:
            raise ValueError("beam major/minor must be provided and positive")
        bmaj_arcsec = raw_major
        bmin_arcsec = raw_minor
    if bmaj_arcsec is None or bmin_arcsec is None:
        raise ValueError("beam major/minor must be provided and positive")
    major = float(bmaj_arcsec)
    minor = float(bmin_arcsec)
