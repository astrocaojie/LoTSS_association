...
from .config import (
    ...
    MARGINAL_PROBABILITY_BOUNDARY,
    MORPHOLOGY_SCORE_WEIGHTS,
    RESOLVED_PROBABILITY_BOUNDARY,
    RESOLVED_SIGNIFICANCE_NORMALIZATION,
)
...
# Morphology supplies PA reliability and compactness evidence to later stages.

def _classification_config(config: dict[str, Any] | None) -> dict[str, Any]:
    out = dict(CLASSIFICATION_DEFAULTS)
...
def _deconv_axes(row: pd.Series) -> tuple[float, float, float]:
    # Catalogue normalization guarantees that underscored size fields are
    # arcseconds.  Raw PyBDSF names are intentionally not interpreted here.
    major = _row_value(row, ["_dc_maj"])
    minor = _row_value(row, ["_dc_min"])
...
def _observed_axes(row: pd.Series, config: dict[str, Any] | None) -> tuple[float, float, float]:
    ...
    major = _row_value(row, ["_maj"], beam_major)
    minor = _row_value(row, ["_min"], beam_minor)
    pa = _row_value(row, ["_pa"], float("nan"))
...
def _axis_ratio_score(
    obs_major: float,
    obs_minor: float,
    beam_major: float,
    beam_minor: float,
    tolerance: float = CLASSIFICATION_DEFAULTS["beam_like_axis_ratio_tolerance"],
) -> float:
    obs_ratio = obs_major / max(obs_minor, 1e-6)
    beam_ratio = beam_major / max(beam_minor, 1e-6)
    if obs_ratio <= 0 or beam_ratio <= 0:
        return 0.0
    return float(np.exp(-abs(np.log(obs_ratio / beam_ratio)) / max(float(tolerance), 1e-6)))
...
def _size_score(
    ...
    beam_major: float,
    beam_minor: float,
    tolerance: float = CLASSIFICATION_DEFAULTS["beam_like_size_tolerance"],
) -> float:
    tolerance = max(float(tolerance), 1e-6)
    major_score = np.exp(-abs(np.log(max(obs_major, 1e-6) / max(beam_major, 1e-6))) / tolerance)
    minor_score = np.exp(-abs(np.log(max(obs_minor, 1e-6) / max(beam_minor, 1e-6))) / tolerance)
...
def beam_like_score(row: pd.Series, config: dict[str, Any] | None = None) -> float:
    ...
    ratio_score = _axis_ratio_score(
        obs_major,
        obs_minor,
        beam_major,
        beam_minor,
        float(cfg["beam_like_axis_ratio_tolerance"]),
    )
    size = _size_score(
        obs_major,
        obs_minor,
        beam_major,
        beam_minor,
        float(cfg["beam_like_size_tolerance"]),
    )
    if np.isfinite(obs_pa):
        ...
        pa_score = 0.5
    values = (ratio_score, size, pa_score)
    return float(
        np.clip(
            sum(weight * values[index] for index, weight in enumerate(MORPHOLOGY_SCORE_WEIGHTS)),
            0.0,
            1.0,
        )
    )
...
def classify_gaussian_component(row: pd.Series, config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    # Downweight PA evidence for unresolved or beam-like Gaussians.
    cfg = _classification_config(config)
    beam_major, beam_minor, _beam_pa = beam_axes_from_config(config)
    obs_major, obs_minor, obs_pa = _observed_axes(row, config)
    ...
    resolved_probability = float(np.clip(resolved_significance / RESOLVED_SIGNIFICANCE_NORMALIZATION, 0.0, 1.0))
    if morphology_class == "resolved":
        resolved_probability = max(resolved_probability, RESOLVED_PROBABILITY_BOUNDARY)
    elif morphology_class == "marginally_resolved":
        resolved_probability = float(
            np.clip(resolved_probability, MARGINAL_PROBABILITY_BOUNDARY, RESOLVED_PROBABILITY_BOUNDARY)
        )
    elif morphology_class == "unresolved":
        resolved_probability = min(resolved_probability, MARGINAL_PROBABILITY_BOUNDARY)
