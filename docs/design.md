...
- PyBDSF Gaussian catalog supplied explicitly with `--gaus-catalog`.
...
- sky position: `RA`, `DEC`, optional errors (degrees)
- pixel position: `Xposn`, `Yposn` (or supported x/y aliases) when sky
  coordinates are unavailable
- fluxes: `Total_flux`, `Peak_flux`
- sizes and angle: `Maj`, `Min`, `PA`, `DC_Maj`, `DC_Min`, `DC_PA`
- code/status: `S_Code`

PyBDSF expresses the four Gaussian size fields (`Maj`, `Min`, `DC_Maj`, and
`DC_Min`) in degrees.  The catalogue normalization layer converts them once to
the internal `_maj`, `_min`, `_dc_maj`, and `_dc_min` fields in arcseconds.
Downstream association and morphology routines therefore never infer a size
unit from a raw catalogue column.  Position angles are kept in degrees with
the PyBDSF convention.

Production entry points validate that every row has a usable sky position or
pixel position, flux information, and shape information. Missing required
fields fail before any output is written; optional identifiers may fall back to
stable row indices.
...
3. **No-match mode**: if neither sky coordinates with a WCS nor explicit pixel
   coordinates are available, matching fails with a clear error. Segmentation
   alone is not published as an association catalogue.
...
- sum image pixel values inside 2 sigma and 2.5 sigma support near the
  component (reported in the input image pixel-value units),
- find peak flux,
