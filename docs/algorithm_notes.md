...
5. Use the constrained graph in `association.py` to form local groups. The
   legacy `graph_merge.py` implementation can be run independently for
   compatibility diagnostics, but does not define the release Stage 1 result.
