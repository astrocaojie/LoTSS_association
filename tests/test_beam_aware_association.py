...
class BeamGeometryTest(unittest.TestCase):
    ...
    def test_candidate_pairs_filter_by_elliptical_distance_and_absolute_cap(self) -> None:
        ...
        config["association"]["max_pair_distance_arcsec"] = 240.0
        beam_angle = sky_pa_to_pixel_angle(-89.25)
        ...
        capped["association"]["max_pair_distance_arcsec"] = 90.0
...
class ConservativeParentAcceptanceTest(unittest.TestCase):
    ...
    def test_min_host_quality_setting_is_consumed(self) -> None:
        row = parent_row(host_evidence="supports_double_lobe", host_quality="medium", bridge_snr_support=True)
        default_result = classify_parent_acceptance(row, {})
        strict_config = {"host_support": {"min_host_quality_for_default_candidate": "high"}}
        strict_result = classify_parent_acceptance(row, strict_config)
        self.assertEqual(default_result["parent_acceptance_class"], "accepted_high_confidence_parent")
        self.assertEqual(strict_result["parent_acceptance_class"], "rejected_parent_candidate")
        self.assertEqual(strict_result["parent_acceptance_reason"], "host_required_for_parent_link")

    def test_midpoint_host_does_not_replace_radio_evidence(self) -> None:
        result = classify_parent_acceptance(
            parent_row(host_evidence="supports_double_lobe", host_quality="medium"),
            {},
        )
        self.assertEqual(result["parent_acceptance_class"], "geometry_only_visual_candidate")

    def test_geometry_failure_has_specific_fallback_reason(self) -> None:
        result = classify_parent_acceptance(
            parent_row(double_lobe_geometry_pass=False, rejection_reason=""),
            {},
        )
        self.assertEqual(result["parent_acceptance_reason"], "parent_geometry_failed")

    def test_endpoint_failure_has_specific_fallback_reason(self) -> None:
        result = classify_parent_acceptance(
            parent_row(endpoint1_is_parent_endpoint_allowed=False, rejection_reason=""),
            {},
        )
        self.assertEqual(result["parent_acceptance_reason"], "parent_endpoint_not_eligible")
